...
import pandas as pd
import requests
import hopsworks                          # ← top-level import (not inside try)
...
# ─────────────────────────────────────────────────────────────
if not API_KEY:
    print("ERROR: OPENWEATHER_API_KEY not set in secrets"); sys.exit(1)
if not HOPSWORKS_API_KEY:
    print("ERROR: HOPSWORKS_API_KEY not set in secrets"); sys.exit(1)
...
try:
    ...
    if r2.status_code == 401:
        print("ERROR: Invalid OpenWeather API key (401)"); sys.exit(1)
    r2.raise_for_status()
...
try:
    ...
    hist["date"] = pd.to_datetime(
        hist["date"], utc=True, errors="coerce"
    ).dt.tz_localize(None)
    ...
    new_row = {
        ...
        # Integer columns — must be int to match Hopsworks bigint schema
...
try:
    ...
    hist_updated = pd.concat(
        [hist,
         new_df.rename(columns={"event_time": "date"}).drop(columns=["row_id"])],
        ignore_index=True,
    )
